...
import torch
import torchvision
from torchvision.transforms import GridDropout
...
# GN归一化number of groups to separate the channels into
...
#正余弦形式的Position Embedding位置信息嵌入空间
...
# 先norm后fn，在执行fn之前执行一个Layer Norm
...
class ResnetBlock(nn.Module):
    def __init__(self, dim, dim_out, *, time_emb_dim = None, groups = 8):
        ...
        self.mlp = nn.Sequential(
            nn.SiLU(),
            nn.Linear3d(time_emb_dim, dim_out)
        ) if exists(time_emb_dim) else None

        self.block1 = Block(dim, dim_out, groups = groups)
        self.block2 = Block(dim_out, dim_out, groups = groups)
        self.res_conv = nn.Conv3d(dim, dim_out, 1) if dim != dim_out else nn.Identity3d()
    ...
    def __init__(
        ...
        if with_time_emb:
            ...
            self.time_mlp = nn.Sequential(
                ...
                nn.Linear3d(dim, time_dim),
                nn.GELU(),
                nn.Linear3d(time_dim, time_dim)
            )
        else:
            time_dim = None
            self.time_mlp = None
        ...
        for ind, (dim_in, dim_out) in enumerate(in_out):
            ...
            self.downs.append(nn.ModuleList([
                ...
                Downsample(dim_out) if not is_last else nn.Identity3d()
            ]))
        ...
        for ind, (dim_in, dim_out) in enumerate(reversed(in_out[1:])):
            ...
            self.ups.append(nn.ModuleList([
                ...
                Upsample(dim_in) if not is_last else nn.Identity3d()
            ]))
...
class GaussianDiffusion3D(nn.Module):
    ...
    def p_sample(self, x, t, clip_denoised=True, repeat_noise=False):
        ...
        # no noise when t == 0
        nonzero_mask = (1 - (t == 0).float()).reshape(b, *((1,) * (len(x.shape) - 2)))
    ...
    def interpolate(self, x1, x2, t = None, lam = 0.5):
        ...
        t_batched = torch.stack([torch.tensor(t, device=device, dtype = torch.long)] * b)
...
class Dataset3D(data.Dataset):
    def __init__(self, folder, image_size, exts = ['jpg', 'jpeg', 'png', 'tiff', 'hdf5']):
        super().__init__()
        self.folder = folder
        self.image_size = image_size
        ...
        self.transform = transforms.Compose([
            ...
            transforms.RandomFlip(dims=[0,1,2]),
            transforms.RandomCrop(image_size),
            transforms.GridDropout(ratio = 0.5,unit_size_min = 50,
...
class Trainer(object):
    def __init__(
        ...
        *,
        ...
        self.ds = Dataset3D(folder, image_size)
